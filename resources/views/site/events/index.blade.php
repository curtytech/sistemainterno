...
<body class="bg-slate-50 font-sans text-slate-900">
  <header class="border-b border-slate-200 bg-white">
    <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-5">
      ...
      <nav class="flex items-center gap-3">
        ...
        <a href="{{ route('site.news.index') }}" class="rounded-full bg-primary px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90">Ver notícias</a>
      </nav>
    </div>
  </header>
  ...
  <main class="mx-auto max-w-7xl px-4 py-12">
    <div class="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-primary">Agenda</p>
        <h1 class="mt-2 text-4xl font-bold text-slate-900">Todos os eventos</h1>
        <p class="mt-3 max-w-2xl text-slate-600">Confira treinamentos, comunicados e ações internas em um só lugar.</p>
      </div>
      <a href="{{ route('site.index') }}#eventos" class="text-sm font-semibold text-primary transition hover:opacity-80">Voltar para a seção da home</a>
    </div>
